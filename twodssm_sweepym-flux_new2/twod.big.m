pl	2	#
		...
		define nxp ($nx+4)
		define nyp ($ny+4)
		define Lxp ($Lx*(1.+2.*$dx))
		define Lyp ($Ly*(1.+2.*$dy))
		define ma 1.1
		if($Lx > $Ly) {define L ($ma*$Lx)} else {define L ($ma*$Ly)}
		set i=1,$nxp*$nyp
                set y=(i - $nyp*int((i-0.5)/$nyp) - 0.5)*$dy
		set x=(int((i-0.5)/$nyp) + 0.5)*$dx
                #set x=(i - $nxp*int((i-0.5)/$nxp) - 0.5)*$dx
		#set y=(int((i-0.5)/$nxp) + 0.5)*$dy
		image($nxp,$nyp) -$Lxp $Lxp -$Lyp $Lyp
		set ix = int(x/$dx)
		set iy = int(y/$dy)
		set image[ix,iy] = $2[i-1]
		#
		limits -$Lxp $Lxp -$Lyp $Lyp
		erase
		box
...
rd	3	#
		...
		set lvx = lg(abs(vx)) if(vx!=0.)
		set lvy = lg(abs(vy)) if(vy!=0.)
		set N2 = N2 if(N2<10&&N2>-10)
		#
		##read {X 1 Y 2 r 3}
plx	2	#
		rd $1
